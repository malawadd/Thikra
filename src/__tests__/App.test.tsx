...
describe('App', () => {
  ...
  it('keeps the chat-mode picker chip visible with "Pick a model" when active model disappears (S2)', async () => {
    ...
    const textarea = screen.getByPlaceholderText('Ask Thikra anything...');
    act(() => {
  ...
  it('renders the no-models strip copy when Ollama is reachable but empty (S2)', async () => {
    ...
    expect(strip.textContent).toContain(
      "Thikra couldn't find any local LLM models",
    );
  ...
  it('shows overlay on visibility show event', async () => {
    ...
    expect(
      screen.getByPlaceholderText('Ask Thikra anything...'),
    ).toBeInTheDocument();
  });
  ...
  it('hides overlay on Escape key and cancels an active /search turn', async () => {
    ...
    expect(invoke).toHaveBeenCalledWith('notify_overlay_hidden');
    expect(screen.queryByPlaceholderText('Ask Thikra anything...')).toBeNull();
    vi.useRealTimers();
  });
  ...
  describe('image integration', () => {
    ...
    it('cancels deferred submit when all images fail', async () => {
      ...
      expect(screen.getByPlaceholderText('Ask Thikra anything...')).toHaveValue(
        'describe',
      );
    });
  });
  ...
  describe('/kite command', () => {
    ...
    it('routes /kite status through run_kite_command instead of ask_ollama', async () => {
      ...
      const textarea = screen.getByPlaceholderText('Ask Thikra anything...');
      act(() => {
        fireEvent.change(textarea, { target: { value: '/kite status ' } });
      });
      await act(async () => {
        fireEvent.keyDown(textarea, { key: 'Enter', shiftKey: false });
      });
      ...
      expect(invoke).toHaveBeenCalledWith(
        'run_kite_command',
        expect.objectContaining({ input: '/kite status' }),
      );
      expect(invoke).not.toHaveBeenCalledWith('ask_ollama', expect.anything());
    });

    it('routes /kite setup through run_kite_command', async () => {
      enableChannelCapture();
      render(<App />);
      await act(async () => {});
      await showOverlay();
      ...
      const textarea = screen.getByPlaceholderText('Ask Thikra anything...');
      act(() => {
        fireEvent.change(textarea, { target: { value: '/kite setup ' } });
      });
      await act(async () => {
        fireEvent.keyDown(textarea, { key: 'Enter', shiftKey: false });
      });
    ...
    it('bypasses the OpenRouter privacy warning for /kite commands', async () => {
      ...
      enableChannelCaptureWithResponses({
        get_settings: {
          ...
          api_key_openrouter: 'sk-or-v1-test',
      ...
      expect(
        screen.queryByText('I understand â€” Send'),
      ).not.toBeInTheDocument();
      expect(invoke).toHaveBeenCalledWith(
        'run_kite_command',
        expect.objectContaining({ input: '/kite status' }),
  ...
  describe('Onboarding', () => {
    ...
    it('does not show onboarding on normal visibility event', async () => {
      ...
      expect(screen.queryByText("Let's get Thikra set up")).toBeNull();
    });
