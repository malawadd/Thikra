...
describe('useOllama', () => {
  ...
  describe('askKite()', () => {
    it('sends /kite commands to the native backend command', async () => {
      const { result } = renderHook(() => useOllama());

      await act(async () => {
        await result.current.askKite('/kite status');
      });
    ...
    it('confirms pending Kite payments and resumes the flow', async () => {
      const confirmSpy = vi
      ...
      await act(async () => {
        await result.current.askKite('/kite call --url https://paid.example');
      });
      ...
      act(() => {
        channel!.simulateMessage({
          ...
          data: {
            action_id: 'payment-1',
            summary: 'Approve Kite payment?',
          },
        });
      });

      expect(confirmSpy).toHaveBeenCalledWith('Approve Kite payment?');
      expect(invoke).toHaveBeenCalledWith('confirm_kite_payment_action', {
        actionId: 'payment-1',
      });
      confirmSpy.mockRestore();
    });

    it('renders advisory fallback guidance when autopilot is unavailable', async () => {
      const { result } = renderHook(() => useOllama());
      ...
      act(() => {
        channel!.simulateMessage({
          ...
          data: {
            ...
            guidance: 'Kite advisory fallback guidance',
          },
        });
        channel!.simulateMessage({ type: 'Done' });
      });
