...
const HELPERS = {
  inference: {
    ollama_url:
      'The web address where Thikra finds your local Ollama server. The default works if you run Ollama on this machine with its standard port. Change this only if you moved Ollama to a different port or another machine.',
    keep_warm:
      'When on, Thikra tells Ollama to keep the active model loaded in GPU memory between conversations, saving the cold-load wait on every open. Set "Release after" to -1 to keep it indefinitely, or pick a timeout in minutes so GPU memory is reclaimed when you stop using Thikra for a while.',
    num_ctx:
  ...
  prompt: {
    system:
      "Your custom personality or instructions for the AI. Leave this empty to use Thikra's built-in secretary personality. The list of slash commands is always added on top.",
  },
  window: {
    overlay_width:
      'How wide the floating Thikra window is, in pixels. Raise for wider input/chat at the cost of more screen space; lower to keep Thikra compact.',
    max_chat_height:
      'The largest the chat window can grow to as conversation gets longer. Raise to see more chat history without scrolling; lower to keep Thikra from taking over your screen.',
    max_images:
  ...
  search: {
    searxng_url:
      "Where Thikra's local search engine (SearXNG) is running. Keep this on 127.0.0.1; pointing it at a remote host leaks every search query.",
    reader_url:
      "Where Thikra's local web-page reader is running. The reader opens promising URLs, strips out ads, menus, and scripts, and hands the clean text back so the AI can read it.",
    searxng_max_results:
    ...
    top_k_urls:
      'How many web pages Thikra actually opens and reads after picking the most promising ones from the search results. Raise for more sources; lower for faster searches.',
    search_timeout_s:
      'How long (in seconds) Thikra waits for SearXNG to come back with search results before giving up. Raise for slow internet connections.',
    reader_per_url_timeout_s:
      'How long (in seconds) Thikra waits for one single web page to load before giving up on it. Raise for slow websites.',
    reader_batch_timeout_s:
      'How long (in seconds) Thikra waits for the whole batch of pages it is reading in parallel to finish. Must be larger than the per-URL timeout.',
    judge_timeout_s:
      'How long (in seconds) Thikra waits for the AI to decide whether the search results are good enough. Raise for slow local AI models.',
    router_timeout_s:
      'How long (in seconds) Thikra waits for the AI to decide whether your question needs a web search. Raise for slow local AI models.',
  },
  gateway: {
    enabled:
      'Enable the local gateway server. When enabled, other apps on your machine can send messages to Thikra through the gateway port.',
  ...
  debug: {
    search_trace_enabled:
      'When on, Thikra writes a detailed trace file for every /search turn. Useful for diagnosing search issues; leave off for normal use.',
  },
} as const;
