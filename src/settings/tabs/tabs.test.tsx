...
describe('ModelTab', () => {
  ...
  it('reflects search_trace_enabled=true from config when the section is expanded', () => {
...
describe('AgentTab', () => {
  ...
  it('renders the Kite Passport section and hydrated status details', async () => {
    await renderAgent();
  ...
  it('saves, verifies, and disconnects Kite settings through Tauri commands', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Save MCP URL' }));
    await waitFor(() =>
      expect(invokeMock).toHaveBeenCalledWith('set_kite_mcp_url', {
        url: 'https://neo.dev.gokite.ai/v1/mcp?token=updated',
      }),
    );

    invokeMock.mockImplementation(async (cmd: string) => {
      if (cmd === 'verify_kite_connection') {
        return {
          connected: true,
          authState: 'ready',
          availableTools: ['get_payer_addr', 'approve_payment'],
          message: 'Kite MCP connection verified.',
        };
      }
      if (cmd === 'get_kite_setup_status') {
        return {
    ...
    await waitFor(() =>
      expect(invokeMock).toHaveBeenCalledWith('verify_kite_connection'),
    );

    fireEvent.click(screen.getByRole('button', { name: 'Disconnect' }));
    await waitFor(() =>
      expect(invokeMock).toHaveBeenCalledWith('disconnect_kite'),
    );
  });

  it('installs the Kite CLI from Settings', async () => {
    invokeMock.mockImplementation(async (cmd: string) => {
      if (cmd === 'get_settings') {
        return {
          kite_mcp_url: '',
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Install Kite CLI' }));
    await waitFor(() =>
