...
describe('COMMANDS registry', () => {
  ...
  it('includes the /kite command', () => {
    const kite = COMMANDS.find((c: Command) => c.trigger === '/kite');
    expect(kite).toBeDefined();
    expect(kite?.label).toBe('/kite');
    expect(kite?.description.length).toBeGreaterThan(0);
  });
