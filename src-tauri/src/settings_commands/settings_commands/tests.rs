...
fn allowed_fields_count_matches_schema_field_count() {
    ...
    // + search(11) + debug(1) + gateway(2) + tts(3) + agent(3) = 30 tunable fields.
    // The active model slug lives in the SQLite app_config table via
    // ActiveModelState, not in TOML. If this assertion fails, the schema has
    // drifted from the allowlist and someone added a field without extending
    // ALLOWED_FIELDS.
